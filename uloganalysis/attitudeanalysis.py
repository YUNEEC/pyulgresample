...
def add_roll_pitch_yaw(df):
    df['T_vehicle_attitude_0__F_roll'], df['T_vehicle_attitude_0__F_pitch'], df['T_vehicle_attitude_0__F_yaw'] = mpd.series_quat2euler(df['T_vehicle_attitude_0__F_q_0'], 
                                            df['T_vehicle_attitude_0__F_q_1'],
                                            df['T_vehicle_attitude_0__F_q_2'],
                                            df['T_vehicle_attitude_0__F_q_3'])
def add_euler_error(df):
    df['T_vehicle_attitude_setpoint_0__F_e_roll'] = mpd.angle_wrap(df['T_vehicle_attitude_setpoint_0__F_roll_body'] - df['T_vehicle_attitude_0__F_roll'])
    df['T_vehicle_attitude_setpoint_0__F_e_pitch'] = mpd.angle_wrap(df['T_vehicle_attitude_setpoint_0__F_pitch_body'] -df['T_vehicle_attitude_0__F_pitch'])
    df['T_vehicle_attitude_setpoint_0__F_e_yaw'] = mpd.angle_wrap(df['T_vehicle_attitude_setpoint_0__F_yaw_body'] -df['T_vehicle_attitude_0__F_yaw'])

def add_vehicle_z_axis(df):
    x = pd.Series(np.zeros(df.shape[0]), index = df['timestamp'], name = 'T_vehicle_attitude_0__F_body_z_axis_x')
    y = pd.Series(np.zeros(df.shape[0]), index = df['timestamp'], name = 'T_vehicle_attitude_0__F_body_z_axis_y')
    z = pd.Series(np.ones(df.shape[0]), index = df['timestamp'], name = 'T_vehicle_attitude_0__F_body_z_axis_z')
...
def add_desired_tilt(df):
    ...
    df['T_vehicle_attitude_setpoint_0__F_tilt_desired'] =  df['T_vehicle_attitude_setpoint_0__F_tilt_desired'].apply(np.arccos)
...
def add_tilt(df):
    ...
    df['T_vehicle_attitude_0__F_tilt'] =  df['T_vehicle_attitude_0__F_tilt'].apply(np.arccos)
...
def add_desired_z_axis(df):
    x = pd.Series(np.zeros(df.shape[0]), index = df['timestamp'], name = 'T_vehicle_attitude_setpoint_0__F_body_z_axis_sp_x')
    y = pd.Series(np.zeros(df.shape[0]), index = df['timestamp'], name = 'T_vehicle_attitude_setpoint_0__F_body_z_axis_sp_y')
    z = pd.Series(np.ones(df.shape[0]), index = df['timestamp'], name = 'T_vehicle_attitude_setpoint_0__F_body_z_axis_sp_z')
